...
//    	GPIO_WriteToOutputPin(GPIOB, GPIO_PIN_NO_12, RESET) ;
    	...
    	// command :- led control < pin no > <value >
    	uint8_t command = 0x54 ;

        SPI_Send(SPI2 , &command, 1) ; // sending the command

    	SPI_Read(SPI2,&dummyread,1) ;	/* for every transmission there is a reception
    								so a dummy read must be done to clear the flag */

        // oonce the confirmation of the command is sent slave will send ack or nack

        	// sending some dummy data to receive the data
        	SPI_Send(SPI2 , &dummybyte, 1) ; // sending dummy data
        	SPI_Read(SPI2,&ackbyte,1) ;	// receive ack byte

        	uint8_t id[30];
        			uint32_t i=0;

        	if (ackbyte == 0xF5) {
        		// send arguments ( pin number and value to be written )
        		//read 10 bytes id from the slave
        					for(  i = 0 ; i < 30 ; i++)
        					{
        						//send dummy byte to fetch data from slave
        						SPI_Send(SPI2,&dummybyte,1);
        						SPI_Read(SPI2,&id[i],1);
        					}
